...
만약 연속체 가설이 참이라면 $\kappa < 2^{\aleph\_0} \rightarrow \kappa = \aleph\_0$이므로 마틴 공리가 성립한다. 그러나 마틴 공리와 연속체 가설의 부정이 무모순적임이 알려져 있다 (이 사실의 증명 또한 강제법을 사용한다. 개요: $2^{\aleph\_0} = \aleph\_2$이도록 강제한 다음, 추가로 $\mathrm{MA}_{\aleph\_1}$을 강제한다.). 따라서 마틴 공리는 연속체 가설의 약화된 버전으로 생각될 수 있다.
...
$\forall$의 정의에 의해 어떤 $x = \sigma\_G$에 대해 $\theta^{M[G]}$가 성립함을 보이면 충분하다.
