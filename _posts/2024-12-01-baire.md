...
> **정의.** $X$가 위상공간이라고 하자. 각 $n \in \mathbb{N}$에 대해 $F\_n$이 내부가 공집합인 $X$의 닫힌집합이라고 하자. $\bigcap F\_n$의 내부 또한 언제나 공집합일 때, $X$를 **베르 공간**이라고 한다.
...
- $\lbrace  q \rbrace$는 내부가 공집합인 닫힌 집합이지만 $\bigcup\_{q \in \mathbb{Q}} \lbrace q\rbrace = \mathbb{Q}$는 내부를 가진다.
