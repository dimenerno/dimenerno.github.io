...
This is an extremely simple yet powerful theorem, but its proof is rather tedious in undergraduate topology texts such as Munkres. However, there is an elegant proof using ultrafilters.
...
In a [previous post](https://dimenerno.github.io/2025/07/18/boolean-algebra), we introduced filters in Boolean algebra structures. Since topological spaces form Boolean algebraic structures when $\cup$ and $\cap$ correspond to disjunction and conjunction respectively, we can define filters on topological spaces.
...
When $\mathcal{C}$ is a prefilter, one can **generate** a filter $\mathcal{F}$ from $\mathcal{C}$ as follows:
...
When $f$ is onto, we have $\mathcal{C} = f(\mathcal{F})$. Also, when $f$ is onto, the following holds (trivially):
...
Let each $X\_i$ be a compact space for $i \in I$. $X$ is compact if and only if any ultrafilter $\mathcal{F}$ on $\prod\_{i \in I}$ converges, and $\mathcal{F}$ converges if and only if $\pi\_i(\mathcal{F})$ converges in $X\_i$. Since $\pi\_i$ is onto, $\pi\_i(\mathcal{F})$ is an ultrafilter, and since $X\_i$ is compact, $\pi\_i(\mathcal{F})$ converges. Hence, $X$ is compact. ■
